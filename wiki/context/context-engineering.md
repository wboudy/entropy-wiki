...
description: Managing token composition during inference for AI agents
---
...
This stems from transformer architecture's n-squared pairwise token relationships. Models trained on shorter sequences have fewer specialized parameters for long-range dependencies, creating a performance gradient rather than a hard cliff.

## Context Compaction Strategies

For long-horizon tasks, compaction summarizes conversations nearing capacity limits and reinitiates new context windows. The technique balances recall and precision - preserving architectural decisions and critical details while discarding redundant outputs. Claude Code implements this by compressing message history while retaining recently accessed files.
...
- Defines "Context Rot" and strategies for mitigating it
- Progressive Disclosure (loading files only when needed) maintains agent IQ over long sessions
- Context Compaction preserves critical decisions while discarding redundant outputs
